...
sub Run {
    ...
    if ( $Param{Action} eq 'PreLogin' ) {
        ...
        #begin recaptcha
        if ( $ConfigObject->Get('Frontend::Output::FilterElementPost')->{ShowGoogleRecaptcha}->{Templates}->{CustomerLogin} )
        {
    ...
    # create new customer account
        ...
        {
            # get reCAPTCHA token
            $GetParams{UserGRecaptcha} = $ParamObject->GetParam(Param => 'g-recaptcha-response') || '';
            if ( !$GetParams{UserGRecaptcha} ) {
                ...
                $LayoutObject->Print(
                    Output => \$LayoutObject->CustomerLogin(
                        Title => 'Login',
                        Message =>
                            Translatable('reCAPTCHA entry failed. Please try again.'),
            ...
            my $SecretKey = $ConfigObject->Get('GoogleCaptcha::SecretKey');
                ...
                $Kernel::OM->Get('Kernel::System::Log')->Log(
                    Priority => 'error',
                    Message =>
                         "$GetParams{UserGRecaptcha}\n\n\n$SecretKey\n\n\n$ENV{'REMOTE_ADDR'}",
                );

            # Verify submission
            my $result = $c->check_answer_v2(
                $SecretKey, $GetParams{UserGRecaptcha},
                $ENV{'REMOTE_ADDR'}
            );
